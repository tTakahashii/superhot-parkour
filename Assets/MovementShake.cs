...
public class MovementShake : MonoBehaviour
{
        ...
        if (shakeEnabled)
        {
            switch (movementManager.playerState)
            {
                case MovementManager.MovementState.ControllerBased:
                    ShakeCamera(movementManager.charController, startingPoint, shakeOffset, speedMultipliers, speedClamp);
                    break;

                case MovementManager.MovementState.PhysicsBased:
                    ShakeCamera(movementManager.rb, startingPoint, shakeOffset, speedMultipliers, speedClamp);
                    break;
            }
        }
    }
    ...
    private void ShakeCamera(Rigidbody rb, Vector3 startingPoint, Vector3 shakeOffset, Vector3 speedMultipliers, float speedClamp)
    {
        ...
        if (velocity > 0.01f)
        {
            ...
            {
                case true:
                    interpolators.y += -(Time.deltaTime * velocity * speedMultipliers.y);
            ...
            {
                case true:
                    interpolators.z += -(Time.deltaTime * velocity * speedMultipliers.z);
